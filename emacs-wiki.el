...
(defvar emacs-wiki-version "2004.04.26-00.16-dev"
         ...
         (save-match-data
           (emacs-wiki-make-link
            ...
            (read-string "Text: "
                         (save-match-data
                           (emacs-wiki-link-unescape
                            (match-string-no-properties 5) t))))))
...
(defun emacs-wiki-make-link (link &optional name)
  ...
  (let ((case-fold-search nil))
    ...
    (when (equal name
                 (emacs-wiki-wiki-link-target link))
      (setq name nil))
...
(defun emacs-wiki-index ()
  ...
  (let ((emacs-wiki-project emacs-wiki-current-project))
    ...
    (goto-char (point-min))
    (funcall (or major-mode 'emacs-wiki-mode))
                  ...
                  (let ((text (match-string 6)))
                    (if (and emacs-wiki-inline-images
                             ...
                             (save-match-data
                               ...
                               (string-match emacs-wiki-image-regexp text)))
                        (save-match-data
                          (emacs-wiki-inline-image (match-beginning 6)
                                                   (match-end 6)
                                                   text
                                                   (match-string 6)))
    ...
    (let* (string
      ...
      (setq string
            (if (null url)
              ...
              (let ((text (emacs-wiki-wiki-visible-name wiki-link))
                    alt)
                (save-match-data
                  (when (string-match "^\\([^ ]+\\)\\s-+\\(.+\\)" text)
                    (setq alt
                          (if (match-string 2 text)
                              (format " alt=\"%s\""
                                      (emacs-wiki-escape-html-string (match-string 2 text)))
                            ""))
                    (setq text (match-string 1 text))))
                (if (save-match-data
                      (string-match emacs-wiki-image-regexp url))
                    (if (string-equal url text)
                        (format "<img src=\"%s\"%s/>" url name)
                      (if (save-match-data (string-match emacs-wiki-image-regexp text))
                          (format "<a href=\"%s\"><img src=\"%s\"%s/></a>"
                                  url text alt)
                        (format "<img src=\"%s\"%s/>" url name)))
                  (if (save-match-data
                        (string-match emacs-wiki-image-regexp text))
                      (format "<a href=\"%s\"><img src=\"%s\"%s/></a>" url text alt)
