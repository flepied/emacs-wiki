...
;;
;; Copyright (C) 2003,2004 Sacha Chua <sacha@free.net.ph>
;;
;;; Commentary:
;; 
...
(defun emacs-wiki-id-highlight ()
  ...
  ;; FIXME: Code duplicated from planner-markup-region should be
  ;; moved into emacs-wiki.el eventually, I guess
  (let ((properties (list 'keymap emacs-wiki-id-keymap)))
    (if (and (fboundp 'overlay-put) (fboundp 'remove-overlays))
        (progn
          (remove-overlays (match-beginning 0) (match-end 0) 'emacs-wiki-id t)
    ...
    (save-window-excursion
      ...
      (let ((value 1))
        ...
        (condition-case err
          ...
          (error
           (progn
             (message "Error reading number from %s. Setting value to 1."
...
If the first element of PAGES is a number, it is used as the ID value.
Otherwise, the next ID value is taken from
...
(defun emacs-wiki-id-parse (identifier)
  ...
  (when (string-match emacs-wiki-id-regexp identifier)
    (append
     (list (string-to-number (match-string-no-properties 1 identifier)))
     (split-string (match-string-no-properties 2 identifier) ","))))

(defsubst emacs-wiki-id-add-page (identifier page)
  "Parse IDENTIFIER and append PAGE to it.  Return the new identifier."
  (emacs-wiki-id-make-id (append (emacs-wiki-id-parse identifier) page)))

(defsubst emacs-wiki-id-remove-page (identifier page)
  "Parse IDENTIFIER and remove PAGE to it.  Return the new identifier."
  (emacs-wiki-id-make-id (delete page (emacs-wiki-id-parse identifier))))

(defun emacs-wiki-id-next-page (current-page parsed-id)
  "Return the page following CURRENT-PAGE in PARSED-ID, or nil if not found."
...
(defun emacs-wiki-id-previous-page (current-page parsed-id)
  "Return the page before CURRENT-PAGE in PARSED-ID, or nil if not found."
...
(defun emacs-wiki-id-visit-next-page (current-page parsed-id)
  "Jump to the ID in the page that follows CURRENT-PAGE based on PARSED-ID."
  (forward-char 1)
  (if (re-search-forward (concat "{{"
                                 (number-to-string (car parsed-id))
                                 ":?[^}]*}}") nil t)
      (goto-char (match-beginning 0))
    (emacs-wiki-id-visit (emacs-wiki-id-next-page current-page parsed-id)
                         parsed-id)))
...
(defun emacs-wiki-id-visit-previous-page (current-page parsed-id)
  ...
  (if (re-search-backward (concat "{{"
                                  ...
                                  ":?[^}]*}}") nil t)
      (goto-char (match-beginning 0))
    (emacs-wiki-id-visit (emacs-wiki-id-previous-page current-page parsed-id)
                         parsed-id t)))
...
(defun emacs-wiki-id-edit-id-at-point ()
  ...
  (if (emacs-wiki-id-at-point)
      (let ((new-pages (read-string "Edit pages: "
            ...
            (old-data (emacs-wiki-id-parse (match-string-no-properties 0)))
            new-id)
        (let ((data (run-hook-with-args-until-success
                     'emacs-wiki-id-prepare-for-update-hook)))
          (setq new-id (emacs-wiki-id-make-id (list
                                               (car old-data)
          ...
          (mapc (lambda (item)
                  (when (emacs-wiki-id-visit item new-id)
                    (if (member item new-id)
                        (run-hooks-with-args-until-success
...
(defvar emacs-wiki-id-keymap
  (let ((map (make-sparse-keymap)))
    ...
    (if (featurep 'xemacs)
        (progn
          (define-key map [(button2)] 'emacs-wiki-id-follow-id-at-mouse)
      ...
      (define-key map [(shift mouse-2)] 'emacs-wiki-id-follow-id-at-mouse))
    map)
